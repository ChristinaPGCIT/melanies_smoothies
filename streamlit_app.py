...
# -------------------------------------
# Customer input: list of ingredients (FRUIT_NAME for UI; SEARCH_ON for API)
my_dataframe = session.table("smoothies.public.fruit_options").select(col('FRUIT_NAME'),col('SEARCH_ON'))
# st.dataframe(data=my_dataframe, use_container_width=True)
# st.stop()

# Convert Snowpark DataFrame to a Panda Dataframe so we can use the LOC function
pd_df = my_dataframe.to_pandas()
# st.dataframe(pd_df)
# st.stop()

# Suggested by AI:  Build options for the multiselect  <-- FIX 3
